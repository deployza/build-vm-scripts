...
# -----------------------------------------------------------------------------
# assess-ui.sh — app deploy script (the <APP_NAME>.sh that vm-startup.sh clones
# and runs as a child at boot). assess-ui is a STATIC UI WAR: no database, no
...
#       sudo bash assess-ui.sh <APP_ENV> 2>&1 | tee /tmp/assess-ui.log
#
...
# <clone>/vm/assess-ui.sh — so the name is already implied.) Only APP_ENV varies
